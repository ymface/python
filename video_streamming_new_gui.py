...
class VideoNetProUltimate:
    ...
    def setup_gui(self):
        ...
        tk.Button(group_conn, text="Server(Rx)", command=self.start_server, bg="#ffcccc").pack(side=tk.LEFT, padx=2)
    ...
    def start_server(self):
        self.server_thread = threading.Thread(target=self.server_listener, daemon=True)
        self.server_thread.start()
        messagebox.showinfo("Server", f"Server listening on {PORT}")
    ...
    def server_listener(self):
        ...
        while True:
            ...
            threading.Thread(target=self.receive_stream, args=(client,), daemon=True).start()
    ...
    def receive_stream(self, client_socket):
        ...
        while True:
            try:
                ...
                if frame is not None:
                    self.display_frame(frame, self.lbl_remote)
